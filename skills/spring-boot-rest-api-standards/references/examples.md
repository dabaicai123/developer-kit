...
public class UserController {
    ...
    @GetMapping
    public Result<UserResponse> getUsers(
            @RequestHeader(value = "Accept-Version", defaultValue = "1.0") String version) {

        if (version.equals("2.0")) {
            return Result.success(v2UserResponse);
        }
        return Result.success(v1UserResponse);
    }
}
```
...
@GetMapping("/{id}")
public Result<UserResponseWithLinks> getUserWithLinks(@PathVariable Long id) {
...
public class AsyncUserController {
    ...
    @GetMapping("/{id}")
    public CompletableFuture<Result<UserResponse>> getUserById(@PathVariable Long id) {
        return asyncUserService.getUserById(id)
                .thenApply(Result::success)
                .exceptionally(ex -> Result.fail(ErrorCode.NOT_FOUND, "User not found"));
    }

    @PostMapping
    public CompletableFuture<Result<UserResponse>> createUser(
            @Valid @RequestBody CreateUserRequest request) {
        return asyncUserService.createUser(request)
                .thenApply(Result::success)
                .exceptionally(ex -> {
                    if (ex.getCause() instanceof BusinessException) {
                        return Result.fail(ErrorCode.BAD_REQUEST, ex.getCause().getMessage());
                    }
...
public class FileController {
    ...
    public ResponseEntity<Resource> downloadFile(@PathVariable String fileName) {
        // File download returns ResponseEntity because it sets Content-Type and Content-Disposition headers
