...
Result<T>                   // Unified result wrapper (project convention)
Result.success(data)        // 200 OK with data
Result.success()            // 200 OK without data (void operations)
Result.fail(code, msg)      // Error response with code and message
```
...
### Pagination Response Format (PageResult)
...
public class GlobalExceptionHandler {
    ...
    public Result<Void> handleValidation(
        ...
        // Handle validation errors, return Result.fail(...)
    }

    @ExceptionHandler(ResourceNotFoundException.class)
    public Result<Void> handleNotFound(
            ResourceNotFoundException ex) {
        // Handle not found, return Result.fail(ErrorCode.NOT_FOUND, ...)
    }

    @ExceptionHandler(Exception.class)
    public Result<Void> handleGeneral(Exception ex) {
        // Handle generic exceptions, return Result.fail(ErrorCode.INTERNAL_ERROR, ...)
    }
}
```
...
@GetMapping
public Result<PageResult<ProductResponse>> getAll(
        @RequestParam(defaultValue = "1") int page,
...
<!-- MyBatis-Plus (for Page / LambdaQueryWrapper) -->
<dependency>
