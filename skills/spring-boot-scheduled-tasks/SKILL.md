...
- Adding `@Scheduled` tasks to a Spring Boot application
- Setting up XXL-Job for distributed scheduling across multiple service instances
...
XXL-Job admin console must be deployed separately — it is a standalone web application. Download from [xuxueli/xxl-job](https://github.com/xuxueli/xxl-job).
...
- **`@Scheduled` does not prevent concurrent execution across multiple service instances** — in distributed deployment, every instance runs the task. Use XXL-Job instead
- **XXL-Job admin console must be deployed separately** — it is a standalone web application, not part of your Spring Boot application
