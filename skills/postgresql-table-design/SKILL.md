...
- `id` BIGINT PRIMARY KEY — use `GENERATED ALWAYS AS IDENTITY` (PostgreSQL) or snowflake ID (application layer)
- `create_time` TIMESTAMPTZ NOT NULL DEFAULT now() — record creation timestamp
- `update_time` TIMESTAMPTZ NOT NULL DEFAULT now() — last modification timestamp; update on every write
- `create_by` BIGINT NOT NULL — ID of the user who created the record
- `update_by` BIGINT NOT NULL — ID of the user who last modified the record
- `deleted_at` TIMESTAMPTZ — soft delete marker; NULL means active, non-NULL means deleted and stores the deletion timestamp
- `version` INTEGER NOT NULL DEFAULT 1 — optimistic lock counter; increment on every update
...
CREATE INDEX ON orders (create_time);
...
- **Soft delete**: `UPDATE orders SET deleted_at = now(), update_by = ? WHERE id = ? AND deleted_at IS NULL`
...
UPDATE orders
SET status = 'PAID', update_time = now(), update_by = ?, version = version + 1
