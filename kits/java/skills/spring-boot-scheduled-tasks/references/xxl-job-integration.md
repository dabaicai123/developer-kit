...
Handler methods are annotated with `@XxlJob("handlerName")`. The handler name must match the JobHandler name configured in the XXL-Job admin console.
...
- For non-numeric keys, use hash-based sharding
- All instances must complete before the task is marked as done in admin console
...
Log notes:
- `XxlJobHelper.log()` writes to both the local log file and the admin console log
- Logs are viewable in admin console: Task Management -> Execution Log -> View Log
- Log files are stored in `logPath` directory and auto-cleaned after `logRetentionDays`
- For large-scale tasks, log key milestones (batch counts, totals) rather than every single item
...
Tasks managed entirely in admin console — code is edited online and stored in admin database. Useful for one-off tasks or quick fixes, but not recommended for production-critical tasks.
