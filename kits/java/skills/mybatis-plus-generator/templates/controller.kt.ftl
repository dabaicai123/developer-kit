...
class ${table.controllerName}(private val ${table.serviceName?substring(0,1)?lower_case}${table.serviceName?substring(1)}: ${table.serviceName}) : ${superControllerClass}() {
<#else>
class ${table.controllerName}(private val ${table.serviceName?substring(0,1)?lower_case}${table.serviceName?substring(1)}: ${table.serviceName}) {
</#if>
...
<#if swagger>
    ...
    fun create(@RequestBody entity: ${entity}): ${entity}? {
        return if (${table.serviceName?substring(0,1)?lower_case}${table.serviceName?substring(1)}.save(entity)) entity else null
    }
