...
**DDD/COLA projects** use the Gateway pattern instead — see `ddd-cola` skill for `OrderGateway` (with `save()` + `update()`) / `OrderGatewayImpl` (in `infrastructure/order/` at domain root, with `gatewayimpl/database/dataobject/` nested sub-packages) pattern.
