...
| `serialPolicy` | Global config / JAVA | Remote serialization: `SerialPolicy.KRYO5` (2.7+, **recommended**) / `SerialPolicy.KRYO` / `SerialPolicy.JAVA` |
| `keyConvertor` | Global config | Key conversion: `KeyConvertor.FASTJSON2` (2.6.5+, **recommended**) / `KeyConvertor.FASTJSON` (legacy) / `KeyConvertor.NONE` (LOCAL only) |
...
- [`jetcache-configuration-reference.md`](jetcache-configuration-reference.md): YAML configuration, serialization, connection configuration
- [`jetcache-api-reference.md`](jetcache-api-reference.md): Cache API, QuickConfig builder, distributed lock API
