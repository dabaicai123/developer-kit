...
**For Maven projects**, add a dedicated `native` profile to keep the standard build clean. See [advanced-config.md](references/advanced-config.md) for full Maven configuration.
...
**For Gradle projects**, apply the `org.graalvm.buildtools.native` plugin. See [advanced-config.md](references/advanced-config.md) for Gradle configuration and build args.
...
**Spring Boot** (3.x+): Spring Boot has built-in GraalVM support with AOT processing. Use RuntimeHintsRegistrar and @RegisterReflectionForBinding — patterns shown in Instructions below.
...
**Quarkus and Micronaut**: These frameworks are designed native-first and require minimal additional configuration. Refer to their official docs for native build guides.
...
**Metadata files** live in `META-INF/native-image/<group>/<artifact>/`. See [advanced-config.md](references/advanced-config.md) for reflect-config.json and resource-config.json formats.
