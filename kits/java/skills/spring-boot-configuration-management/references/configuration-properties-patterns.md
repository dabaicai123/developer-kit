...
Spring Boot 3.x uses Java records for immutable configuration. Records naturally support constructor binding — `@ConstructorBinding` is unnecessary for single-constructor classes (the common case with records). For multi-constructor classes, `@ConstructorBinding` is still required to specify which constructor to use for binding.
...
public class DatabaseProperties {
    ...
    // Note: constructor binding requires -parameters compiler flag
    // (auto-set by spring-boot-starter-parent or Spring Boot Gradle plugin)
...
## @DefaultValue for Constructor Parameter Defaults

Spring Boot 3.x provides `@DefaultValue` to set default values for constructor parameters and record components:
...
app:
  server:
    host: 192.168.1.100  # overrides "localhost" default
    port: 9090           # overrides 8080 default
    # timeout not set — uses 30s default
```

`@DefaultValue` also works with `@DurationUnit` and `@DataSizeUnit` for unit-aware defaults:
