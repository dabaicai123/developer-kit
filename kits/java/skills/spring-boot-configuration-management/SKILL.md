...
Use `spring.config.import=nacos:` in `application.yml` (Spring Boot 3.x recommended approach) for centralized, dynamically refreshed configuration. Legacy `bootstrap.yml` requires `spring-cloud-starter-bootstrap`. Annotate beans with `@RefreshScope` for dynamic refresh. Use `ConfigService.addListener()` for programmatic change detection.
...
// | SpEL support          | Yes                           | No                         |
// | Dynamic refresh       | With @RefreshScope            | With @RefreshScope         |
...
- `@RefreshScope` creates CGLIB proxy objects — cannot be used on Java records (which are implicitly `final`). Use a mutable class with getters/setters for `@RefreshScope` beans. Be careful with `@PostConstruct` initialization order; the proxy may not be fully initialized during construction
...
- Constructor binding is enabled when a class has a single parameterized constructor. For multi-constructor classes, `@ConstructorBinding` is still required. Annotate a parameterized constructor with `@Autowired` to opt out of constructor binding. Records naturally have one constructor and work seamlessly. Constructor binding requires the `-parameters` compiler flag (auto-set by `spring-boot-starter-parent`).
...
- In Spring Boot 3.x, the recommended approach is `spring.config.import=nacos:` in `application.yml`. Legacy `bootstrap.yml` requires `spring-cloud-starter-bootstrap` dependency
