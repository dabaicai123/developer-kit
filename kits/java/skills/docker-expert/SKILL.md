...
| Slow Spring Boot startup | `/dev/random` blocking, classpath scanning | `-Djava.security.egd=file:/dev/./urandom`, layered JAR, GraalVM native |
| Connection failures | Missing health-check-based startup ordering | `depends_on: condition: service_healthy`, HikariCP retry |
| Image over 500 MB | Full JDK base, build tools in production | Use `eclipse-temurin:21-jre`, multi-stage, distroless |
