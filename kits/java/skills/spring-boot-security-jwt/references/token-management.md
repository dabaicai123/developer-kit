...
- NOT store `authorities`, `roles`, or `tenantId` in refresh tokens — re-fetch from database during refresh
- NOT use sliding refresh in production — stolen tokens can be used indefinitely; use rotating refresh
- NOT skip JTI tracking — required for revocation and reuse detection
- NOT omit TTLs on Redis keys — tokens persist forever and consume memory without TTLs
...
public String generateAccessToken(UserDetails userDetails, Long userId) {
    return Jwts.builder()
        ...
        .claim("tenantId", getCurrentTenantId())
        .signWith(signingKey)
        .compact();
}
```
...
> NOT include `authorities`, `roles`, or `tenantId` in refresh tokens. Authorization claims are re-fetched from the database during refresh.
...
public class RedisTokenStore {
    ...
    public boolean isRefreshTokenValid(String jti) {
        return Boolean.TRUE.equals(redisTemplate.hasKey(REFRESH_TOKEN_KEY_PREFIX + jti));
    }
    ...
    public void blacklistAccessToken(String jti, long remainingMs) {
        redisTemplate.opsForValue().set(BLACKLIST_KEY_PREFIX + jti, "1", Duration.ofMillis(remainingMs));
    }
...
> Sliding refresh is less secure — stolen refresh tokens can be used until expiration. Use rotating refresh for production.
...
> NOT omit TTLs on Redis keys — keys persist forever without TTLs, consuming memory indefinitely.
...
- [JJWT 0.13.0 Documentation](https://github.com/jwtk/jjwt)
