...
> Do NOT use `@EnableGlobalMethodSecurity`. It is deprecated since Spring Security 6.0 and removed in 6.4+. Always use `@EnableMethodSecurity`.
...
> `hasRole('ADMIN')` automatically adds the `ROLE_` prefix. It matches an authority `ROLE_ADMIN`. If your JWT stores `authorities` as `["ADMIN"]`, you must either prefix them with `ROLE_` during token generation, or use `hasAuthority('ADMIN')` instead.
...
public class JwtClaimAccessor {
    ...
    /**
     * Extract claims from the current SecurityContext authentication.
     * Works with JwtAuthenticationToken or UsernamePasswordAuthenticationToken
     * where details contain JWT claims.
     */
    public Object getClaim(String claimKey) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        if (auth == null) return null;

        // If using JwtAuthenticationToken with claims in credentials
        if (auth.getCredentials() instanceof Claims claims) {
            return claims.get(claimKey);
        }

        // If claims are stored in authentication details
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
            ...
            // Custom authentication provider (for DaoAuthenticationProvider)
            .authenticationProvider(authenticationProvider)
...
public class AdminUserController {
    ...
    @GetMapping
    @PreAuthorize("hasAuthority('user:read')")     // ADMIN + user:read
    public Result<List<User>> listUsers() {}

    @PostMapping
    @PreAuthorize("hasAuthority('user:create')")   // ADMIN + user:create
...
public CorsConfigurationSource corsConfigurationSource() {
    ...
    // Or use pattern matching for subdomains
...
public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
    return http
        .cors(Customizer.withDefaults())                 // Enable CORS with the bean above
        .csrf(AbstractHttpConfigurer::disable)
...
> Never use `allowedOrigins("*")` with `allowCredentials(true)` in production. Spring Security rejects this combination. Use specific origins or `allowedOriginPatterns`.

## CSRF Configuration

For stateless JWT APIs, CSRF is disabled because tokens are sent via the Authorization header (not cookies), which makes CSRF attacks irrelevant:
...
If you send JWTs via cookies (not the Authorization header), CSRF protection IS needed:
...
> The `CookieCsrfTokenRepository.withHttpOnlyFalse()` allows the SPA to read the CSRF token from the cookie and include it in request headers. This is required for cookie-based JWT authentication.
...
public class CustomPermissionEvaluator implements PermissionEvaluator {
    ...
    public boolean hasPermission(Authentication authentication,
        ...
        // Target-type-specific permission logic
...
> Use `@PostAuthorize` sparingly. The method body executes first, then authorization is checked. This can leak data through side effects (caching, logging). Prefer `@PreAuthorize` when possible.
...
> `@PostFilter` iterates the returned collection and removes elements that do not match the expression. `filterObject` represents each element in the collection.
...
> `@PreFilter` removes elements from the input collection that do not match the expression before the method executes. `filterObject` represents each element in the input collection.
