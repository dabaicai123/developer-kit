...
public class ResilientEventPublisher {
    ...
    // Async publish with CompletableFuture callback (Spring Kafka 3.x)
    public void publishEventAsync(String topic, String key, Object payload) {
        CompletableFuture<SendResult<String, Object>> future = kafkaTemplate.send(topic, key, payload);
        future.whenComplete((result, ex) -> {
            if (ex == null) {
                log.info("Event published to topic {} at offset {}", topic, result.getRecordMetadata().offset());
            } else {
