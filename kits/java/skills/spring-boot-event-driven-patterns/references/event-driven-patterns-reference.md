...
### ApplicationEvent
Base class for Spring events (no longer required — Spring supports any POJO as an event object). Extending `ApplicationEvent` is optional; plain objects work with `ApplicationEventPublisher` directly.
...
# Kafka client metrics are auto-exported when spring-kafka + actuator are on classpath
# Key Micrometer metrics: kafka.producer.record-send-rate, kafka.producer.record-error-rate,
# kafka.consumer.records-consumed-rate, kafka.consumer.records-lag
```
...
- **spring-boot-rest-api-standards** - Domain events in REST API operations
- **spring-boot-rest-api-standards** - Event notifications via webhooks
- **spring-boot-tdd** - Testing event-driven systems
