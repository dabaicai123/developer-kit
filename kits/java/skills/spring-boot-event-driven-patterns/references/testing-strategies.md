...
static class DltConsumer {
    ...
    @KafkaListener(topics = "product-events-dlt", groupId = "test-dlt-consumer")
...
void shouldSendFailedEventsToDLQ() {
    ...
    // Then — verify message arrives in DLQ via test consumer
    await().atMost(10, TimeUnit.SECONDS)
        .untilAsserted(() -> {
            assertThat(dltConsumer.getDltMessage()).isNotNull();
        });
}
```
