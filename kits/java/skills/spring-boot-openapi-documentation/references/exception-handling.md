...
## Unified Result Error Response

Use `Result<T>` for all API responses including errors. See `spring-boot-rest-api-standards/references/unified-result-pattern.md` for full definition.
...
public class GlobalExceptionHandler {
    ...
    @ExceptionHandler(BookNotFoundException.class)
    @Operation(hidden = true)
    public Result<Void> handleNotFound(BookNotFoundException ex) {
