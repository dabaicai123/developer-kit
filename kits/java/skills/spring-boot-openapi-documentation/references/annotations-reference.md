...
Core annotations for COLA/DDD Spring Boot 3.5.x projects. All `description` fields use Chinese.
...
// Method level override — specific endpoint
...
Auto-generates `page`, `size`, `sort` parameters. COLA projects typically use `@RequestParam` instead (see `cola-openapi-patterns.md`).
...
SpringDoc auto-generates constraints from **Jakarta** validation (Jakarta EE 10). No extra `@Schema` needed for these:
...
**NOT** using `javax.validation.*` annotations — Spring Boot 3.5.x requires `jakarta.validation.*`. `javax.*` annotations are not detected by SpringDoc.