...
```java
// Standalone properties POJO (SpringBoot 3.x pattern)
@ConfigurationProperties(prefix = "async")
public class AsyncExecutorProperties {
    private Map<String, ExecutorProps> executors = new HashMap<>();
...
// Configuration class — uses properties via constructor injection
@Configuration
@EnableAsync
...
public class AsyncExecutorConfig {
    ...
    @Bean("ioExecutor")
    public Executor ioExecutor() {
        ExecutorProps props = properties.getExecutors().get("io");
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
    ...
    @Bean("computeExecutor")
    public Executor computeExecutor() {
        ExecutorProps props = properties.getExecutors().get("compute");
        ThreadPoolTaskExecutor executor = new ThreadPoolTaskExecutor();
        executor.setCorePoolSize(props.corePoolSize());
        executor.setMaxPoolSize(props.maxPoolSize());
        executor.setQueueCapacity(props.queueCapacity());
