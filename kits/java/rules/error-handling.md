...
3. **Use unified `Result<T>` wrapper** — all API responses must follow `code/msg/data` format. Use `Result.success(data)` for success, `Result.fail(intCode, msg)` for errors. Never return bare objects or `ResponseEntity`.
