...
1. **Define tokens with `@theme` using OKLCH** - use `@theme` in CSS to define color, spacing, and typography tokens. Use OKLCH color space for perceptual uniformity. New Tailwind v4 work should be CSS-first; keep `tailwind.config.js` only as a legacy migration bridge via `@config`.
...
3. **Reference tokens in component classes** - `bg-primary`, `text-on-surface`, `border-border`. Reference semantic tokens, never hardcoded utility values like `bg-blue-500`.
...
- New Tailwind v4 tokens in `tailwind.config.js` - configuration should live in CSS `@theme` blocks; use `@config` only for legacy compatibility
...
- Adding new theme tokens to JavaScript config - Tailwind v4 should be CSS-first; keep JS config only for legacy `@config` compatibility
