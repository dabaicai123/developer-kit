...
public Result<Void> headUser(@PathVariable Long id) { ... }

@OptionsMapping("/users")
public Result<Void> optionsUsers() { ... }
```
...
> **Note:** Our project convention uses `Result<T>` wrapper instead of `ResponseEntity` for all Controller methods. See unified-result-pattern.md.
...
public class GlobalExceptionHandler {
    ...
    public Result<Void> handleValidationException(
        ...
        // Handle validation errors globally, return Result.fail(...)
    }
}
```
...
public class GlobalExceptionHandler {
    ...
    public Result<Void> handleNotFound(NoHandlerFoundException ex) {
        return Result.fail(ErrorCode.NOT_FOUND, "Resource not found");
    }
...
public class UserController {
    ...
    public CompletableFuture<Result<User>> getUserAsync(@PathVariable Long id) {
