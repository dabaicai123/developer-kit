...
- Understand event-driven architecture concepts and patterns
- Design domain events to decouple aggregates and services
- Evaluate event sourcing vs simple domain events
- Understand CQRS (Command Query Responsibility Segregation)

> For Spring Boot implementation details (ApplicationEventPublisher, @TransactionalEventListener, Kafka integration), see `spring-boot-event-driven-patterns`.
