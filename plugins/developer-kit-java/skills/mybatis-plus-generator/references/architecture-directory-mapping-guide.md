...
> **权威参考**: 与 `ddd-cola` skill 保持一致，遵循 COLA V5 官方命名约定。

**Directory Structure:**
```
src/main/java/{package}/
├── adapter/                          # 适配器层（对齐 ddd-cola）
│   ├── controller/                   # HTTP/RPC/message inbound handlers
│   │   └── UserController.java
│   ├── scheduler/                    # Scheduled tasks
│   └── dto/                          # Web DTO（Request/Response）
│       ├── UserCreateRequest.java
│       └── UserResponse.java
├── app/                              # 应用层（COLA V5 使用 app，不是 application）
│   ├── executor/                     # 执行器（CQRS）
│   │   ├── command/                  # 命令执行器
│   │   │   └── UserCreateCmdExe.java
│   │   └── query/                    # 查询执行器
│   │       └── UserGetQryExe.java
│   ├── service/                      # 应用服务（Service 接口）
│   │   └── UserAppService.java
│   └── service/impl/                 # 应用服务实现（ServiceImpl）
...
│   │   │   └── User.java
│   │   └── valueobject/             # 值对象
│   ├── gateway/                      # 领域网关接口（COLA 使用 gateway，不是 repository）
│   │   └── UserGateway.java
│   ├── service/                      # 领域服务
│   └── ability/                      # 题域能力
└── infrastructure/                   # 基础设施层
    ├── persistence/                  # 持久化实现
    │   ├── mapper/                   # MyBatis Mapper 实现
    │   │   └── UserMapper.java
    │   └── entity/                   # 持久化实体（DO）
    │       └── UserDO.java
    ├── external/                     # 外部 API 客户端
...
- BO (App Model) → `{package}/app/executor/` 或 `{package}/app/model/`
...
**COLA V5 命名约定**:
- 应用层包名为 `app`（不是 `application`）
- 仓储接口命名为 `gateway`（不是 `repository`）
- 控制器放在 `adapter/controller/`（不是 `adapter/web/controller/`）
- 使用 `Cmd` 后缀表示命令对象，`Qry` 后缀表示查询对象
- 使用 `Exe` 后缀表示执行器
