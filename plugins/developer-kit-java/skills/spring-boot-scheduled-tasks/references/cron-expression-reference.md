...
- Spring cron starts with seconds; Quartz cron also starts with seconds
- Spring day-of-week: both 0 and 7 mean Sunday. Quartz day-of-week: 1 = Sunday, 7 = Saturday (Quartz convention)
- Quartz/XXL-Job has an optional 7th field for year
- Spring does NOT support the year field
...
**Rules for `?`:**
- You MUST use `?` in either day-of-month or day-of-week — they cannot both be `*`
- `?` means "no specific value" — use it when you specify one date field but not the other
- Example: `0 0 12 1 * ?` = noon on the 1st of every month (day-of-week is irrelevant, use `?`)
...
| `0 0 12 * * MON-FRI` | Weekdays at noon | Yes | Yes (use MON-SUN) |
...
### Chinese business common patterns
...
|---------|--------|-------------------|
| Fields | 6 (seconds through day-of-week) | 7 (seconds through year, year optional) |
| Year field | Not supported | Optional 7th field |
| Day-of-week | 0 and 7 both = Sunday | 1 = Sunday, 7 = Saturday |
| Day-of-week names | MON, TUE, WED, THU, FRI, SAT, SUN | MON, TUE, WED, THU, FRI, SAT, SUN (same names) |
| `L` support | Supported in day-of-month and day-of-week | Same |
| `W` support | Supported in day-of-month | Same |
| `#` support | Supported in day-of-week | Same |
| `?` requirement | Must use `?` in day-of-month or day-of-week when one is specified | Same |
...
- Spring: `0 0 9 ? * 1` = Sunday at 9 AM (0 and 7 both = Sunday)
- Quartz/XXL-Job: `0 0 9 ? * 1` = Sunday at 9 AM (1 = Sunday in Quartz)
- Recommended: use named weekdays (MON-FRI) to avoid confusion