...
3. Generate the following files:
   - **Entity**: `XxxEntity.java` with `@TableName`, `@TableId`, `@TableLogic`
   - **Mapper**: `XxxMapper.java` extending `BaseMapper<XxxEntity>`
   - **Service**: `XxxService.java` extending `IService<XxxEntity>`
   - **ServiceImpl**: `XxxServiceImpl.java` extending `ServiceImpl<XxxMapper, XxxEntity>`
...
6. Include pagination (`Page<>`) and soft delete (`@TableLogic`) support